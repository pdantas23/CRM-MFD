// GET /api/pedido-itens/[idVhsys]
// Retorna itens de um pedido: primeiro tenta o jsonb dados do espelho,
// com fallback para GET /pedidos/{id}/produtos na API VHSYS.
//
// Verifica via createClient() (com RLS) que o pedido é visível ao usuário
// logado. Sem acesso → 403 (não 404, para não vazar info).
...
export async function GET(
  ...
  // Se RLS bloqueou ou não existe, retornar 403 — não cair no fallback VHSYS
  if (espelhoError || !espelho) {
    return NextResponse.json({ erro: "Acesso negado ou pedido não encontrado." }, { status: 403 });
  }
